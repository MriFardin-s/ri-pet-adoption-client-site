        ...
        <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                >
                    ...
                    <div className="flex gap-6 items-center select-none text-slate-700 dark:text-zinc-300 font-semibold text-sm">
                        <label className="flex items-center gap-2.5 cursor-pointer group">
                            <input 
                                type="checkbox" 
                                className="checkbox checkbox-secondary checkbox-sm border-pink-300 rounded-lg checked:bg-pink-500"
                                onChange={() => setSpecies(prev => prev.includes("Dog") ? prev.filter(s => s !== "Dog") : [...prev, "Dog"])} 
                            /> 
                            <span className="group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">Dogs</span>
                        </label>
                        <label className="flex items-center gap-2.5 cursor-pointer group">
                            <input 
                                type="checkbox" 
                                className="checkbox checkbox-secondary checkbox-sm border-pink-300 rounded-lg checked:bg-pink-500"
                                onChange={() => setSpecies(prev => prev.includes("Cat") ? prev.filter(s => s !== "Cat") : [...prev, "Cat"])} 
                            /> 
                            <span className="group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">Cats</span>
                        </label>
                    </div>
                </motion.div>
