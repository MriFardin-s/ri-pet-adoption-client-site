    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-xl">
        <div className="p-6 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-center">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-xl font-bold"><RxCross2 /></button>
        </div>
              ...
              {requests.map((req) => (
                ...
                {/* {console.log(req)} */}
                    <div className="flex flex-col gap-1 mt-2">
                      ...
                      {req.message && (
                        <div className="text-xs text-slate-600 dark:text-zinc-400 bg-slate-100 dark:bg-zinc-800 px-2 py-1 rounded w-fit italic">
                          Note: {req.message}
                        </div>
                      )}
                    </div>

                    <span className="block mt-2 text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400">
