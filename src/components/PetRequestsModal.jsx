...
import { HiXMark, HiOutlineCalendar, HiOutlineEnvelope, HiOutlineUser, HiOutlineChatBubbleLeft } from "react-icons/hi2";
...
export default function PetRequestsModal({ isOpen, onClose, pet, onStatusUpdated }) {
    ...
    const handleAction = async (requestId, action) => {
        ...
        try {
            ...
            const finalStatus = targetStatus === "rejected" ? "available" : "adopted";
            onStatusUpdated(pet._id, finalStatus);

            toast.success("Request updated");
            if (action === "APPROVED") onClose();
                                    ...
                                    <div key={request._id} className="p-4 border border-slate-100 dark:border-zinc-800/60 rounded-xl flex flex-col gap-3.5 bg-slate-50/50 dark:bg-zinc-800/20 shadow-xs">
                                        <div className="space-y-3 text-sm text-slate-700 dark:text-zinc-300">
                                            ...
                                            <div className="flex items-start gap-2.5">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="font-semibold">{request.pickupDate ? request.pickupDate : "Not specified"}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-start gap-2.5">
                                                <HiOutlineChatBubbleLeft className="text-slate-400 dark:text-zinc-500 mt-0.5 text-base" />
                                                <div>
                                                    <span className="font-bold text-slate-400 dark:text-zinc-500 text-xs uppercase tracking-wider block">Message</span>
                                                    <p className="font-semibold ">{request.message ? `"${request.message}"` : "No message provided"}</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="flex gap-2 justify-end mt-1 items-center">
                                            {showButtons ? (
                                                ...
                                                <span className={`px-3 py-1.5 rounded-lg text-xs font-black uppercase border ${requestStatus === "approved" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-rose-50 text-rose-600 border-rose-100"}`}>
                                                    {requestStatus}
                                                </span>
                                            )}
                                        </div>
